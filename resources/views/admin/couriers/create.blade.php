...
@section('title', 'E-PHARMA - Ajouter livreur')
@section('page_title', 'Ajouter un livreur')
...
<x-admin.card title="Informations livreur">
    <form method="POST" action="{{ route('manager.couriers.store') }}" class="space-y-4">
        @csrf

        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Nom</label>
                <x-admin.input name="name" value="{{ old('name') }}" />
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Téléphone</label>
                <x-admin.input name="phone" value="{{ old('phone') }}" />
            </div>
        </div>

        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Email</label>
            <x-admin.input name="email" type="email" value="{{ old('email') }}" />
        </div>
